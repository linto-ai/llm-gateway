...
Meeting summaries get a new catalog of five services with rich DOCX templates,
template rendering becomes pluggable (tables, repeated rows, mind maps), the
admin controls the order of the service list, and the flavor system prompt is
finally sent to the model.

Upgrade notes:

- Migration 012 (`services.display_order`). Existing services get 100.
- New Python dependency: Pillow (mind map rendering).
- Seeding at API start no longer overwrites anything. The new catalog prompts
  and templates are added, existing prompts, templates and services are left
  as they are. The catalog services are only created when a model is named
  with `SEED_CATALOG_MODEL` (or `python -m app.seeds.base_seed --catalog <model>`).
- To make "Compte rendu" the first summary offered by LinTO Studio and Meet,
  give it position 0 (the seeded catalog does it) and leave the other services
  at 100 or higher.

## Meeting catalog

Five services built for video-conference meetings, one flavor and one template
each, written for mistral-small and in French:

| Service | Position | Content |
|---|---|---|
| Compte rendu | 0 | Full minutes: summary, participants, topics, decisions, proposals, actions, figures, discussion, open points |
| Points clés | 10 | One-page brief: takeaways, decisions, actions |
| Tableau de suivi | 20 | Minutes as a follow-up table (topic, discussion, outcome, owner) with status pictograms |
| Brief commercial | 30 | Customer meeting: context, needs, objections, budget and timing, next steps |
| Note technique | 40 | Technical meeting: architecture, choices and their reasons, risks, tasks |

- The prompts share the same rules: participants are the speakers of the
  transcript, never the people only mentioned; nothing invented; figures and
  deadlines kept as said; a proposal is not a decision; answer in the language
  of the meeting; plain Markdown, no code fence, no text around the document.
- All five use a shared extraction prompt (`linto-extraction-champs`) that fills
  the template placeholders from the generated summary: lists as JSON objects,
  explicit default values, no invention.
- Each service ships its DOCX template (`templates/default/linto-*.docx`):
  infographic header (title, key figures, participants, decisions, actions,
  mind map depending on the template) above `{{output}}`, Inter fonts embedded
  (SIL OFL, `templates/assets/fonts`). The templates are generated by
  `scripts/templates/*.py` and checked by `scripts/templates/check_docx.py`.
  Their description is the text LinTO Studio shows in the export menu.
- The service description is short ("Compte rendu", "Points clés"...) since
  LinTO Studio uses it as the tab label.

## Seeding

- `app.seeds.base_seed` adds the catalog prompts and templates at every API
  start and never modifies an existing row. A prompt whose content differs from
  the seed is kept (`--update-prompts` overwrites library prompts on purpose).
- `--catalog <model_identifier>` or `SEED_CATALOG_MODEL` creates the missing
  catalog services, flavors and template links on an existing model. Existing
  services are left untouched. A missing model logs an error instead of failing
  the whole seed.
- Catalog definitions live in `seeds/prompts/linto-*` (manifest and prompt
  text) and `seeds/services/*` (manifest: position, flavor parameters, prompts
  and template), documented in `seeds/README.md`.

## Service order

- `display_order` on services (migration 012, 0 to 100000, default 100).
  `GET /services` sorts by ascending position, then newest first. LinTO Studio
  shows its tabs in that order with no change on its side, and a client that
  takes the first service gets the one the admin put first.
- Editable through the API (create and update) and on the admin service form
  ("Position dans la liste").

## Template renderers

Rendering features a template opts into are now plugins, one module each under
`app/services/template_renderers`, registered with `@register` and discovered
automatically. Each plugin declares its trigger in the template, what it
expects from the service prompt and what to write in the placeholder
instruction. `docs/TEMPLATE_RENDERERS.md` is generated from that registry.

Two families:

- Output renderers act on `{{output}}`:
  - `output_tables`: Markdown tables of the output get a real table style,
    column widths, alignment and coloured status symbols.
  - `hidden_sections`: sections of the output read by the extraction but not
    printed in the document (e.g. meeting data used by the header).
- Placeholder renderers act on the extracted values:
  - `repeated_rows`: a table row with `{{list.field}}` placeholders is repeated
    once per extracted object (actions, decisions, figures...).
  - `rich_values`: a multi-line value alone in its paragraph becomes bullets,
    `**bold**` pairs are rendered in any paragraph, repeated rows included.
  - `mindmap`: a `{{mindmap_...}}` placeholder alone in its paragraph or cell
    is extracted as an indented outline and drawn as a mind map image (central
    topic, coloured branches, details), sized to the page or the cell.

The extraction run by the worker at the end of a job now goes through the same
renderers as the export, so list and mind map fields are requested in the
right format in both paths.

## PDF export

- `pdf_lock` on job PDF export: sets PDF permissions (no editing, no copying,
  printing allowed) with a random, discarded permission password. Honoured by
  most readers, ignored by converters.
- `pdf_footer_note`: small centered line added to every page footer (200
  characters max).

## Fixes

- The flavor system prompt was never sent when the flavor also had a user
  prompt: the backend loaded one or the other, so single-pass, iterative and
  reduce calls ran without it. Both are now sent, and the system prompt counts
  in the token budget. A flavor with only a system prompt keeps using it as the
  template.
- A code fence wrapping the whole LLM answer (```` ```markdown ```` ... ```` ``` ````)
  is removed before the result is stored, so LinTO Studio no longer shows raw
  Markdown in its editor.
- Services `metadata` (e.g. the icon shown by LinTO Studio and Meet) was
  dropped on create and update: the API field never reached the
  `service_metadata` column.
- Admin: pages mounted before the runtime config was loaded ran their queries
  with an unconfigured API client, the query stayed paused and lists such as
  Services showed empty. The app now renders once the config is known.
- Admin: extracted template fields with no translation (`titre`,
  `mindmap_sujets`...) raised a missing message error in the console for each
  field; they now show a readable label from their name.

## Cleanup

Removed code that nothing imported or called: the unused
`metadata_extraction_service` (duplicate of the export and worker extraction),
13 unused functions and 4 unused schemas.
