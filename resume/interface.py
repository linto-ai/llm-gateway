...
class Interface:
    ...
    def generate_resume(self, cr_type : str, model_name : str, transcription : Transcription):
        ...
        trans.apply_map(self.api_key, self.api_base, read_file_to_string(PROMPTS_DIR+'clean.txt'), max_call=10, model="meta-llama-3-8b-instruct")
        if self.logger:
    ...
    def generate_cri(self, transcription : Transcription, model_name : str) -> list[dict]:
        transcription.apply_map(self.api_key, self.api_base, PROMPTS_DIR+'cri.txt', max_call=10, model="meta-llama-3-8b-instruct")
        return transcription.transcription

    def generate_cra(self, transcription : Transcription, model_name : str) -> list[dict]:
        transcription.apply_map(self.api_key, self.api_base, PROMPTS_DIR+'cra.txt', max_call=10, model="meta-llama-3-8b-instruct")
        return transcription.transcription

    def generate_cred(self, transcription : Transcription, model_name : str) -> list[dict]:
        transcription.apply_map(self.api_key, self.api_base, PROMPTS_DIR+'cred.txt', max_call=10, model="meta-llama-3-8b-instruct")
        return transcription.transcription
