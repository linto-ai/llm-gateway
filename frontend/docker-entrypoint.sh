...
if [ -n "$BASE_PATH" ]; then
    ...
    # Replace placeholder in all JS files (standalone output)
    find /app/.next -type f -name "*.js" -exec sed -i "s|${PLACEHOLDER}|${BASE_PATH}|g" {} + 2>/dev/null || true
    ...
    find /app/.next -type f -name "*.js" -exec sed -i "s|${PLACEHOLDER}||g" {} + 2>/dev/null || true
